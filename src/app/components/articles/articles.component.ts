...
export class ArticlesComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subscriptionArticleList = this.store
      .select('articles')
      .subscribe(
        ArticlesStore =>
          (this.dataSource = new MatTableDataSource(ArticlesStore.articulos))
      );
  }
  ...
  editOrViewModal() {
    ...
    dialogRef.afterClosed().subscribe((result: Article) => {
      if (!!result) {
        this.store.dispatch(
          new ArticuloUpdateAction(result, result.id_articulos)
        );
      }
    });
  }
